...
__global__ void mandelKernel(int* result, float lowerX, float lowerY, float stepX, float stepY, int iter, int resX, int resY, int pitch) {
    ...
    //
    int mem_group = resX/16;
    for(int col=0; col < mem_group; col++)
    {      
		    int thisY = threadIdx.y + blockIdx.y * blockDim.y;
		    int thisX = (threadIdx.x + blockIdx.x * blockDim.x) * mem_group + col;
...
void hostFE (float upperX, float upperY, float lowerX, float lowerY, int* img, int resX, int resY, int maxIterations)
{
    ...
    cudaMallocPitch((void **)&d_result, &pitch, sizeof(int)*resX, resY);
    
    // 定義kernel的執行配置
    dim3 blockSize(16, 16);
    dim3 gridSize(1, (resY + blockSize.y - 1) / blockSize.y);
    // 執行kernel
